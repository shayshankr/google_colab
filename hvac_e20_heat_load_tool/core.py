...
def e20_heat_load(inputs: ZoneInputs) -> Dict[str, float]:
    ...
    occupant_sensible_w, occupant_latent_w = inputs.occupancy * 75.0, inputs.occupancy * 55.0
    ...
    fa_latent_kw = 1.2 * fa_m3s * 2501.0 * max(0.0, w_out - w_in)
    ...
    supply_air_cmh = ((grand_sensible_w / 1000.0) / (1.2 * tr_minus_ts)) * 3600.0
...
def _demo() -> None:
    ...
    if not args.input_json:
        raise SystemExit("Provide user values using --input-json <file>. Use --print-template to generate the expected format.")
    ...
    print(json.dumps(e20_heat_load(inputs), indent=2))
